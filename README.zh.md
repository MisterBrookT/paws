...
3. 将 [`hooks/kiro/paws-signal.sh`](hooks/kiro/paws-signal.sh) 配置为 `stop` 和 `userPromptSubmit` hooks：
   ...
   ```
4. 装一个游戏（`brew install vitetris` 或 `cargo install --git https://github.com/MisterBrookT/jump-high`），重载 Kaku（CMD+Shift+R），按 CMD+G。

## 游戏

Tetris · [Jump High](https://github.com/MisterBrookT/jump-high) · 🌍 地球Online（现实世界支线任务）· 🎲 随机轮换
